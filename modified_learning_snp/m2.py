...
xy_train=np.loadtxt('train_qmean.csv',delimiter=', ',dtype=np.float32)

xy_val=np.loadtxt('val_qmean.csv',delimiter=', ',dtype=np.float32)
np.random.shuffle(xy_val)
x_val=xy_val[0:2000,1:-2].reshape(-1,seq_leng,1)
y_val=xy_val[0:2000,-2:]

test=open('test_qmean.csv','r')
...
writer=tf.summary.FileWriter("./logs/mean_loss")
...
ret=open('ret_mean.txt','w')
Y_real=[]
Y_pred=[]
accu=0
for i in range(int(5050/batch_size)):
...
print('accuracy : ',accu/int(5050/batch_size))
