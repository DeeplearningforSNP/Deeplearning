...
xy_train=np.loadtxt('data_for_train3.csv',delimiter=', ',dtype=np.float32)
xy_val = xy_train[0:100]
xy_train = xy_train[100:540]
'''
...
''''''
# print(xy_val[0:500,-2:])
x_val=xy_val[0:100,1:-2].reshape(-1,seq_leng,1)
y_val=xy_val[0:100,-2:]
print(len(xy_train),len(xy_val))
''''''
xy_test=np.loadtxt('data_for_test3.csv',delimiter=', ',dtype=np.float32)
np.random.shuffle(xy_test)

x_test=xy_test[0:240,1:-2].reshape(-1,seq_leng,1)
y_test=xy_test[0:240,-2:]
...
for epoch in range(200):
    for i in range(int(440/batch_size)):
...
#    print(sess.run([Y[i][0],tf.argmax(prediction,1)[i]],feed_dict={Y:y_test,X:x_test}))
    if sess.run(Y[i][0],feed_dict={Y:y_test})==1:
        if sess.run(tf.argmax(prediction,1)[i],feed_dict={X:x_test})==1:
            ret_matrix=ret_matrix+[[0,0],[1,0]]
        else:
            ret_matrix=ret_matrix+[[0,0],[0,1]]
    else:
        if sess.run(tf.argmax(prediction,1)[i],feed_dict={X:x_test})==0:
            ret_matrix=ret_matrix+[[0,1],[0,0]]
        else:
            ret_matrix=ret_matrix+[[1,0],[0,0]]
