...
xy_test=np.loadtxt('',delimiter=', ',dtype=np.float32)
...
logits=layers.fully_connected(fc_out,out_dim,activation_fn=tf.nn.relu,
        weights_initializer=layers.xavier_initializer(),biases_initializer=tf.zeros_initializer())
...
for i in range(int(np.ceil(6053/batch_size))):
...
print('accuracy : ',accu/int(np.ceil(6053/batch_size)))
